...
def register():
    ...
    if request.method == 'POST':
        ...
        # Check duplicates before doing anything
...
def new_task():
    u = current_user()
    ws = active_workspace()
    if ws and get_ws_role(ws, u) == 'viewer':
        flash('Viewers cannot create tasks.', 'error')
...
def inject_globals():
    u = current_user()
    notifs=[]; unread_notifs=0; overdue_count=0; ws=None; my_workspaces=[]; ws_role=None
    if u:
        ...
        ws_role       = get_ws_role(ws, u) if ws else None
    can_admin_ws = ws_role in ('owner', 'admin')
    can_write_ws = ws_role in ('owner', 'admin', 'member')
    return dict(now=datetime.utcnow(), notifs=notifs, unread_notifs=unread_notifs,
                ...
                ws_role=ws_role, can_admin_ws=can_admin_ws, can_write_ws=can_write_ws)
